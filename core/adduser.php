 ...
 elseif($_SESSION['insert']!=1){
    ...
    if($data2['titype']==2){
        ...
        if($_SESSION["insert"]!=1){
           ...
           mysql_close($conn);
           echo "<script>window.location.href=\"addanswer.php\";</script>";
     }
    else{
    ...
    $time=time();
       if(!empty($row['starttime'])){
        $sqllijian="update user set starttime='$time' where info='$info'";
        mysql_query($sqllijian,$conn);
    }else{
         $sqllijian="update user set temptime=123 where info='$info'";
         mysql_query($sqllijian,$conn);
    }
    if($rowtem['addtest']==1){
        $time=$rowtem['endtime'];
