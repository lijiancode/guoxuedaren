...
$objPHPExcel->setActiveSheetIndex(0)  
            ->setCellValue('A1', '“电信杯”第三届山东省青少年“国学达人”挑战赛总决赛（加试赛）')  
            ->setCellValue('A2', '姓名')  
            ...
            ->setCellValue('E2', '分数')  
            ->setCellValue('F2', '用时')  
            ->setCellValue('G2', '加试赛理解或运用性正确率')
            ->setCellValue('H2', '加试赛常识性正确率')
            ->setCellValue('I2', '排名');
//数据库连接  
...
$sqlgroups="select * from adddetail";  
    ...
    {  
        ...
        {  
            ...
            $objPHPExcel->setActiveSheetIndex(0)              
                        ...
                        ->setCellValue($s6, $data['time'])  
                        ->setCellValue($s7, $data['type3'])
                        ->setCellValue($s8, $data['type4'])    
                        ->setCellValue($s9, $count-2);  
//                         $objPHPExcel->getActiveSheet()->getStyle('$s1')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
                      $objPHPExcel->getActiveSheet()->getStyle('A' . ($count) . ':I' . ($count))->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        }  
...
header('Content-Disposition: attachment;filename="国学达人加试赛(全部).xls"');  
header('Cache-Control: max-age=0');

// $objPHPExcel->getActiveSheet()->getStyle('A4:D7')->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
